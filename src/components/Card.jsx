...
const Card = ({ title, project, client, language, preview_url, image }) => {
  return (
    <div className="w-full sm:w-80 md:w-full lg:w-[32rem] mx-auto p-4 bg-[#dde9f0] shadow-md rounded-lg mt-6">
      {/* Title */}
      <h1 className="text-lg sm:text-xl md:text-2xl font-semibold text-center text-[#ff9f05] mb-4">
        {title}
      </h1>

      {/* Project and Client Details */}
      <div className="flex flex-col sm:flex-row justify-between items-center mb-2 text-sm sm:text-base">
      ...
      <div className="overflow-hidden rounded-lg">
        <a href={preview_url} target="blank">
          <img
            className="w-full h-40 sm:h-52 md:h-64 lg:h-72 object-cover rounded-lg transform transition-transform duration-500 ease-in-out hover:scale-105"
            src={image}
            alt={title}
          />
        </a>
